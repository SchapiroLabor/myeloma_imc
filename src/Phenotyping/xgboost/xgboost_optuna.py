...
base_path = '/gpfs/bwfor/work/ws/hd_bm327-myeloma_standal_ws/phenotyping/uncorrected/xgboost/standard/complete'
data_path = ('/gpfs/bwfor/work/ws/hd_bm327-myeloma_standal_ws/phenotyping/uncorrected/manual_phenotypes_standard_lineage_encoded.csv')
...
def preprocess_data(filepath):
    ...
    data.drop(columns=["index", "Y_centroid", "X_centroid", "lineage"], inplace=True)
...
def main():
    ...
    def objective(trial):
        ...
        for train_index, test_index in kf.split(X_train_val, y_train_val):
            ...
            model.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=50, verbose=1, sample_weight=sample_weights)
    ...
    dump(best_model, os.path.join(base_path, 'best_xgb_model_complete.joblib'))
    best_model.save_model(os.path.join(base_path, 'best_xgb_model_complete.json'))
    fig = optuna.visualization.matplotlib.plot_param_importances(study)
    plt.savefig(os.path.join(base_path, 'optuna_parameter_importance.png'), dpi=300)
    plt.close(fig)
